...
function doGet(e){
  try{
    ...
    let result;
    if(action==='health')result={ok:true,service:'DongwonFC V14.1 Google Sheets API',initialized:props_().getProperty('INITIALIZED')==='true',time:new Date().toISOString()};
...
function initHeaders_(){const specs={
 ...
 [SHEETS.SPECIAL]:['회원번호','경조사','경조사일자','부과금액','구분','상태'],
 [SHEETS.SETTINGS]:['항목','값'],
 [SHEETS.AUDIT]:['일시','사용자','작업','내용']};
 Object.keys(specs).forEach(name=>{const sh=ss_().getSheetByName(name);sh.getRange(1,1,1,specs[name].length).setValues([specs[name]]);sh.setFrozenRows(1);sh.getRange(1,1,1,specs[name].length).setFontWeight('bold').setBackground('#1d4ed8').setFontColor('#ffffff')})}
...
function replaceMembers_(members){ensureSheets_();const mSh=ss_().getSheetByName(SHEETS.MEMBERS),rSh=ss_().getSheetByName(SHEETS.REGULAR),sSh=ss_().getSheetByName(SHEETS.SPECIAL);[mSh,rSh,sSh].forEach(sh=>{if(sh.getLastRow()>1)sh.getRange(2,1,sh.getLastRow()-1,sh.getLastColumn()).clearContent()});
 ...
 if(visible.length){mSh.getRange(2,1,visible.length,6).setValues(visible.map(m=>[String(m.memberNo||'').padStart(3,'0'),m.name||'',m.role||'',m.phone||'',m.jersey||'',Boolean(m.regularExempt)]));rSh.getRange(2,1,visible.length,2).setValues(visible.map(m=>[String(m.memberNo||'').padStart(3,'0'),Number(m.regularPaid)||0]));const rows=[];visible.forEach(m=>(m.specialItems||[]).forEach(i=>rows.push([String(m.memberNo||'').padStart(3,'0'),i.event||'',i.date||'',Number(i.amount)||0,i.category||'current',i.status||'미납'])));if(rows.length)sSh.getRange(2,1,rows.length,6).setValues(rows)}
...
function readSpecial_(){const sh=ss_().getSheetByName(SHEETS.SPECIAL),n=sh.getLastRow(),map={};if(n>=2)sh.getRange(2,1,n-1,Math.max(6,sh.getLastColumn())).getValues().forEach(r=>{if(!r[0]||!r[1])return;const no=String(r[0]).padStart(3,'0'),isLegacy=!r[4]&&r[2]&&['납부','미납','-','본인경조사','면제'].includes(String(r[2]));(map[no]||(map[no]=[])).push(isLegacy?{event:String(r[1]),date:'',amount:0,category:'current',status:String(r[2]||'미납')}:{event:String(r[1]),date:String(r[2]||''),amount:Number(r[3])||0,category:String(r[4]||'current'),status:String(r[5]||'미납')})});return map}

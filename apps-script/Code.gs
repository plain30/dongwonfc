...
function doGet(e){
  try{
    ...
    let result;
    if(action==='health')result={ok:true,service:'DongwonFC V13 Google Sheets API',initialized:props_().getProperty('INITIALIZED')==='true',time:new Date().toISOString()};
...
function uploadBegin_(q){requireSession_(q.token);const id=cleanUploadId_(q.uploadId),total=Math.max(1,Number(q.total)||1);const cache=CacheService.getScriptCache();cache.put('UP_META_'+id,JSON.stringify({total:total,token:q.token}),600);return{ok:true,uploadId:id,total:total}}
function uploadChunk_(q){requireSession_(q.token);const id=cleanUploadId_(q.uploadId),idx=Number(q.index);if(!Number.isInteger(idx)||idx<0)throw new Error('잘못된 업로드 조각입니다.');CacheService.getScriptCache().put('UP_'+id+'_'+idx,String(q.data||''),600);return{ok:true,index:idx}}
function uploadCommit_(q){
  const session=requireSession_(q.token),id=cleanUploadId_(q.uploadId),cache=CacheService.getScriptCache(),raw=cache.get('UP_META_'+id);if(!raw)throw new Error('업로드 세션이 만료되었습니다. 다시 시도하세요.');const meta=JSON.parse(raw);if(meta.token!==q.token)throw new Error('업로드 권한이 없습니다.');let encoded='';for(let i=0;i<meta.total;i++){const c=cache.get('UP_'+id+'_'+i);if(c===null)throw new Error('업로드 데이터 일부가 누락되었습니다.');encoded+=c;}const p=decodePayload_(encoded);p.token=q.token;const result=route_(p);cache.remove('UP_META_'+id);for(let i=0;i<meta.total;i++)cache.remove('UP_'+id+'_'+i);return result;
}

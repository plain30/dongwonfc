const STORE_KEY = 'dongwonfc_v9_1_data';
const SESSION_KEY = 'dongwonfc_v9_1_session';
...
function normalizeMember(m, settings){
  const regularDue=(Number(settings.monthlyFee)||0)*(Number(settings.annualMonths)||12);
  const regularPaid=Math.max(0,Number(m.regularPaid)||0);
...
function memberTable(rows){
 return `<div class="table-wrap"><table><thead><tr><th>회원번호</th><th>성명</th><th>직책</th><th>회원회비 납부금액</th><th>회원회비 내야할금액</th><th>회원회비 남은금액</th><th>특별회비 납부금액</th><th>특별회비 내야할금액</th><th>특별회비 남은금액</th><th>종합 납부율</th></tr></thead><tbody>${rows.map(m=>`<tr><td><b>${m.memberNo}</b></td><td>${escapeHtml(m.name)}</td><td>${escapeHtml(m.role)}</td><td>${won(m.regularPaid)}</td><td><b>${won(m.regularDue)}</b></td><td>${won(m.regularRemain)}</td><td>${won(m.specialPaid)}</td><td><b>${won(m.specialDue)}</b></td><td>${won(m.specialRemain)}</td><td><span class="pill ${combinedRate(m)>=80?'ok':'warn'}">${combinedRate(m)}%</span></td></tr>`).join('')}</tbody></table></div>`;
}
function feeTable(rows){
 return `<div class="table-wrap"><table><thead><tr><th>회원번호</th><th>성명</th><th>납부금액</th><th>내야할금액</th><th>남은금액</th><th>납부율</th></tr></thead><tbody>${rows.map(m=>`<tr><td><b>${m.memberNo}</b></td><td>${escapeHtml(m.name)}</td><td>${won(m.regularPaid)}</td><td><b>${won(m.regularDue)}</b></td><td>${won(m.regularRemain)}</td><td><span class="pill ${pct(m.regularPaid,m.regularDue)>=80?'ok':'warn'}">${pct(m.regularPaid,m.regularDue)}%</span></td></tr>`).join('')}</tbody></table></div>`;
}
...
function downloadCSV(data){
 const header=['회원번호','성명','직책','연락처','회원회비납부','회원회비내야할금액','회원회비남은금액','특별회비납부','특별회비내야할금액','특별회비남은금액'];
 const lines=[header.join(',')].concat(data.members.map(m=>[m.memberNo,m.name,m.role,m.phone,m.regularPaid,m.regularDue,m.regularRemain,m.specialPaid,m.specialDue,m.specialRemain].map(x=>`"${String(x??'').replace(/"/g,'""')}"`).join(',')));
 downloadFile('dongwonfc_members_v9_1.csv','\ufeff'+lines.join('\n'),'text/csv');
}
...
function parseFeesSheet(wb,data){
 const rows=sheetRows(wb,'2026');rows.forEach(r=>{const name=String(r[7]||'').trim(),m=data.members.find(x=>x.name===name);if(!m)return;if(typeof r[8]==='number')m.regularPaid=Math.max(0,r[8]);if(typeof r[11]==='number')m.specialPaid=Math.max(0,r[11])});

import {api,requireRole,logout} from './api.js?v=13.2.0';
import {DEFAULT_SETTINGS,visibleMembers,normalizeMember,won,pct,donut,memberTable,feeTable,specialTable,workbook,parseMembersSheet,applyFeesSheet,applySpecialSheet,esc} from './core.js?v=13.2.0';
...
function specialView(){
  ...
  const paid=sum('specialPaid'),due=sum('specialDue'),remain=Math.max(0,due-paid),events=[...new Set(members.flatMap(m=>(m.specialItems||[]).map(i=>i.event)).filter(Boolean))];
  return `<div class="dashboard-grid special-kpis">
    ${metric('경조사 발생',`${events.length}건`,`1건당 ${won(settings.specialFeePerEvent)}`)}
    ${metric('총 내야할금액',won(due),'회원별 본인경조사 제외')}
...
function eventSummary(events){
  ...
  return `<div class="event-summary-list">${events.map(ev=>{const rows=members.map(m=>(m.specialItems||[]).find(i=>i.event===ev)).filter(Boolean),target=rows.filter(i=>i.status!=='본인경조사').length,paid=rows.filter(i=>i.status==='납부').length;return `<div class="event-summary-row"><div><strong>${esc(ev)}</strong><span>${paid}/${target}명 납부</span></div><div class="event-progress"><span style="width:${target?Math.round(paid/target*100):100}%"></span></div><b>${target?Math.round(paid/target*100):100}%</b></div>`}).join('')}</div>`;
}
...
async function uploadSpecial(file){if(!file)return;try{setStatus('특별회비 저장 중','loading');const wb=await workbook(file),next=applySpecialSheet(wb,members);await saveAll(next,'특별회비 업로드',file.name);alert('특별회비를 공동 서버에 반영하고 검증했습니다.');await refresh(false)}catch(e){alert(e.message);setStatus('저장 실패','offline')}}
...
function downloadBackup(){const blob=new Blob([JSON.stringify({version:'13.2.0',exportedAt:new Date().toISOString(),settings,members},null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`dongwonfc-v13-2-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url)}

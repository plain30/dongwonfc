import {api,requireRole,logout} from './api.js?v=14.0.0';
import {DEFAULT_SETTINGS,visibleMembers,normalizeMember,won,pct,donut,memberTable,feeTable,specialTable,workbook,parseMembersSheet,applyFeesSheet,applySpecialSheet,esc} from './core.js?v=14.0.0';
...
function specialView(){
  ...
  return `<div class="dashboard-grid special-kpis">
    ${metric('2026년 경조사',`${events.filter(e=>e.category!=='arrears').length}건`,events.some(e=>e.category==='arrears')?'전년도 미수금 항목 별도 포함':'업로드 파일의 날짜·건별금액 반영')}
...
function eventSummary(events){
  ...
  return `<div class="event-summary-list">${events.map(ev=>{const key=`${ev.category||'current'}|${ev.date||''}|${ev.event}|${ev.amount||0}`,rows=members.map(m=>(m.specialItems||[]).find(i=>`${i.category||'current'}|${i.date||''}|${i.event}|${i.amount||0}`===key)).filter(Boolean),target=rows.filter(i=>!['본인경조사','면제','해당없음'].includes(i.status)).length,paid=rows.filter(i=>i.status==='납부').length,rate=target?Math.round(paid/target*100):100,label=ev.category==='arrears'?'전년도 미수금':'2026년';return `<div class="event-summary-row"><div><strong>${esc(ev.event)}</strong><span>${label} · ${esc(ev.date||'날짜 미기재')} · ${won(ev.amount)} · ${paid}/${target}명 납부</span></div><div class="event-progress"><span style="width:${rate}%"></span></div><b>${rate}%</b></div>`}).join('')}</div>`;
}
function uploadView(){return `<div class="grid grid-2"><div class="card upload-box"><h3>회원정보 업로드</h3><p class="muted">회원번호 000과 탈퇴회원은 자동 제외됩니다.</p><input type="file" id="membersFile" accept=".xlsx,.xls"></div><div class="card upload-box"><h3>회비내역 업로드</h3><p class="muted">면제·EXEMPT 회원은 회비면제로 처리합니다.</p><input type="file" id="feesFile" accept=".xlsx,.xls"></div><div class="card upload-box"><h3>특별회비 업로드</h3><p class="special-file-note">현재 사용 중인 ‘2025-2026년 동원FC 특별회비현황’ 양식을 그대로 읽습니다. 납부·미납·본인경조사와 날짜·건별금액을 자동 반영합니다.</p><input type="file" id="specialFile" accept=".xlsx,.xls"></div><div class="card"><h3>초기 자료 및 백업</h3><p class="muted">처음 설치할 때 포함된 기본자료를 공동 스프레드시트에 등록할 수 있습니다.</p><div class="toolbar"><button class="btn dark" id="seedBtn">기본자료 서버 등록</button><button class="btn" id="backupBtn">JSON 백업 다운로드</button></div></div></div>`}
